class PureController < ApplicationController
  def users
    ...
    @users = User.limit(100).offset(params[:skip]).order("id")
    render :json => @users.to_json(:except=>except)
  end

  def photos
    @photos = Photo.limit(100).offset(params[:skip]).order("id").includes(:user)
    render :json => @photos.to_json(:only=>:created_at, :methods=>[:url, :user_username])
  end

  def posts
    except = [:updated_at, :subject_id, :usernames, :file_types]
    @posts = Post.limit(100).offset(params[:skip]).order("id").includes(:user)
  ...
  def relationships
    ...
    @relationships = Relationship.limit(100).offset(params[:skip]).order("id").includes(:user1, :user2)
